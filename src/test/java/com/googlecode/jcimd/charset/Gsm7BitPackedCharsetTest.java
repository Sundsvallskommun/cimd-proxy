...
class Gsm7BitPackedCharsetTest {
	...
	void decodesHelloWorld() throws Exception {
		...
		final ByteBuffer byteBuffer = ByteBuffer.wrap(new byte[] {
	...
	void encodesAlphaNumericCharacters() throws Exception {
		final String string = "It is easy to send text messages. 123";
		...
		final byte[] actuals = encoder.encode(in).array();
	...
	void encodesEscapedCharacters() {
		...
		final ByteBuffer out = ByteBuffer.allocate(string.length() * 2);
		encoder.encode(in, out, true);
		final byte[] actuals = out.array();
		for (int i = 0; i < expecteds.length; i++) {
			assertThat(actuals[i]).withFailMessage("at element " + i).isEqualTo(expecteds[i]);
		}
	}
	...
	void usesQuestionMarkToEncodeCharactersOutsideGsmRange() {
		final CharBuffer in = CharBuffer.wrap("\u604F\u7D59");
	...
	void encodeAndDecode() throws Exception {
		final CharBuffer charBuffer1 = CharBuffer.wrap("abc123{}[]");
		final CharBuffer charBuffer2 = decoder.decode(encoder.encode(charBuffer1));
	...
	void trailingCommercialAtIsIgnored() throws Exception {
		...
		final ByteBuffer byteBuffer = ByteBuffer.wrap(bytes);
		final CharBuffer charBuffer2 = decoder.decode(byteBuffer);
